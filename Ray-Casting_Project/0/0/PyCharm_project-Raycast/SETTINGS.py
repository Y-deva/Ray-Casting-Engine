import math
import text_map as textmap
text_map = textmap.text_map
start = textmap.start

'''Game settings'''
FPS = 200
SIZE = WIDTH, HEIGHT = (1280, 768 - 15)
...
map_size = (scale_map * map_cols, scale_map * map_rows)
scale_map_player = (map_size[0] / (tile_size * map_cols))
map_position = (0, HEIGHT - scale_map * map_rows - 40)


def re_map():
	global map_rows, map_position, map_cols, scale_map, map_size, scale_map_player, player_pos, text_map, start
	text_map = textmap.text_map
	start = textmap.start
	map_size = (WIDTH // 4.54, HEIGHT // 4.5)
	map_rows = len(text_map)
	...
	scale_map_player = (map_size[0] / (tile_size * map_cols))
	map_position = (0, HEIGHT - scale_map * map_rows - 40)
	player_pos = round((start[0] + 0.5) * tile_size), round((start[1] + 0.5) * tile_size)
...
'''menu'''
menu_size = (WIDTH // 24, WIDTH // 24)
menu_open_size = (WIDTH // 17, HEIGHT // 4)